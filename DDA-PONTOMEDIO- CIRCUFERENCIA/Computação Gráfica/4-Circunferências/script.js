...
function drawCircleUsingMidPoint(xc, yc, r) {
    ...
    function plotCirclePoints(xc, yc, x, y) {
        setPixel(xc + x, yc + y); // Quadrante 1
        addCoordToSidebar(xc + x, yc + y, count); 
        count++;

        setPixel(xc - x, yc + y); // Quadrante 2
        addCoordToSidebar(xc - x, yc + y, count); 
        count++;

        setPixel(xc + x, yc - y); // Quadrante 4
        addCoordToSidebar(xc + x, yc - y, count); 
        count++;

        setPixel(xc - x, yc - y); // Quadrante 3
        addCoordToSidebar(xc - x, yc - y, count); 
        count++;

        setPixel(xc + y, yc + x); // Quadrante 5
        addCoordToSidebar(xc + y, yc + x, count); 
        count++;

        setPixel(xc - y, yc + x); // Quadrante 6
        addCoordToSidebar(xc - y, yc + x, count); 
        count++;

        setPixel(xc + y, yc - x); // Quadrante 8
        addCoordToSidebar(xc + y, yc - x, count); 
        count++;

        setPixel(xc - y, yc - x); // Quadrante 7
        addCoordToSidebar(xc - y, yc - x, count); 
        count++;
    }
